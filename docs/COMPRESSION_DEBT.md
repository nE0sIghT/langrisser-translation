...
| RU-011a | L1 shared table, `shared.txt` | part 1 | Closed. Seven labels restored once the shared tiling freed the space: "Командир империи", "Великий дракон", "Живые доспехи", "Вампир-лорд", "Демон-лорд", "Гельгизер", "Соколиный лорд". |
| RU-012 | L1 scenarios 1 and 12, `chunk_000.txt`, `chunk_011.txt` | part 7 `0`, `5/1`, `5/23`, `5/31`, `5/40`, `5/59`, `5/60` | Closed. Both cards' narration and all six reworded lines restored to their full wording; the sword is again passed down "from generation to generation" and Digos again besieges Baldea "with an army". |
...
| RU-011 | L1 shared table, `shared.txt` | part 1 | Eight unit labels are still shorter than the Japanese: `サーペントロード` is "Змеелорд", `ドラゴンロード` "Драколорд", `ハイロード` "Владыка", `マスターディーノ` "Дино", `ネクロマンサー` "Некрос", `サー・ガリウス` "Галиус", `シカ族` "Сика", `ストーンゴーレム` "Голем". The table sits in all 21 chunks, so a byte here costs 21. |
| RU-013 | L2 scenario 10, `chunk_009.txt` | `56` | Leon's order to Laird carries a line reading `音声２６４７` — a voice cue the developers left in the string, which the window draws like any other line. The Russian drops it: transcribing it would leave Japanese on screen, and translating it would state something the scene does not say. |
